...
class TestProcessorRedisIntegration:
    ...
    @pytest.mark.asyncio
    async def test_load_trades_dataframe_from_redis(self, processor, mock_redis_adapter):
    ...
    @pytest.mark.asyncio
    async def test_load_available_symbols_from_redis(self, processor, mock_redis_adapter):
        ...
        assert processor._FinPlotChartProcessor__current_available_symbol_name_set == set(mock_symbols)

    @pytest.mark.asyncio
    async def test_fallback_to_database_when_redis_unavailable(self, processor, mock_redis_adapter):
        ...
        processor_module.g_globals.get_postgres_db_session_maker.return_value = mock_session_maker
    ...
    @pytest.mark.asyncio
    async def test_load_all_derived_data_from_redis(self, processor, mock_redis_adapter):
        """Тест загрузки всех производных данных из Redis."""
        symbol_id = SymbolId.BTC_USDT
        
        # Вызываем метод
        await processor._FinPlotChartProcessor__load_all_derived_data_from_redis(symbol_id)
        ...
        mock_redis_adapter.load_order_book_volumes_data.assert_called_once_with(symbol_id)
