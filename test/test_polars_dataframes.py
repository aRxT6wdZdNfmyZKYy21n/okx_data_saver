...
def test_polars_dataframes():
    ...
    from cpp_data_processor import BollingerBands, CandleData, RSIData, VelocityData
    ...
    assert isinstance(bollinger_df, pl.DataFrame), (
        'Bollinger должен быть Polars DataFrame'
    )
    assert isinstance(rsi_df, pl.DataFrame), 'RSI должен быть Polars DataFrame'
    assert isinstance(velocity_df, pl.DataFrame), (
        'Velocity должен быть Polars DataFrame'
    )
