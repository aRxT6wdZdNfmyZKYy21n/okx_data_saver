...
def test_data_structures():
    ...
    try:
        ...
        logger.info(f"✅ TradeData created: trade_id={trade.trade_id}, price={trade.price}")
        
        # Test CandleData
        candle = cpp_data_processor.CandleData(50000.0, 50100.0, 49900.0, 50050.0, 1000.0, 
                                              12345, 12350, 1000, 2000, 6)
        logger.info(f"✅ CandleData created: open={candle.open_price}, close={candle.close_price}")
        ...
        logger.info(f"✅ ProcessingResult created: success={result.success}, time={result.processing_time_seconds}")
...
def test_simple_calculations():
    ...
    try:
        ...
        logger.info(f"✅ Bollinger Bands calculated: {len(bollinger.upper_band)} values")
        ...
        for i in range(100):
            trade = cpp_data_processor.TradeData(
                i + 1, 50000.0 + i, 0.1, i % 2 == 0, 
                base_time.replace(microsecond=i * 1000)
            )
            trades.append(trade)
        
        candles_map = candles_processor.process_trades(cpp_data_processor.SymbolId.BTC_USDT, trades)
...
def test_performance():
    ...
    try:
        ...
        # Convert trades to Python format
        trades_dict = {
        ...
        result = processor.process_trades_data(cpp_data_processor.SymbolId.BTC_USDT, trades_dict)
        end_time = time.time()
