...
from main.save_final_data_set.schemas import (
    OKXDataSetRecordData,
)
...
from main.show_plot_2.globals import (
    g_globals,
)
from main.show_plot_2.gui.window import (
    FinPlotChartWindow,
)
...
class FinPlotChartProcessor:
    __slots__ = (
        ...
        '__current_available_data_set_idx_set',
        '__current_available_symbol_name_set',
        '__current_data_set_idx',
    ...
    def __init__(
        ...
        self.__current_available_data_set_idx_set: set[int] | None = None
        self.__current_available_symbol_name_set: set[str] | None = None
        self.__current_data_set_idx: int | None = None
    ...
    async def get_current_available_data_set_idxs(
        self,
    ) -> list[str] | None:
        current_available_data_set_idx_set = self.__current_available_data_set_idx_set

        if current_available_data_set_idx_set is None:
            return None
        ...
        return list(
            map(
                str,
                sorted(
                    current_available_data_set_idx_set,
                ),
            )
        )
    ...
    def get_current_data_set_idx(
        self,
    ) -> int | None:
        return self.__current_data_set_idx
    ...
    async def update_current_data_set_idx(
        self,
        value: int,
    ) -> bool:
        current_available_data_set_idx_set = self.__current_available_data_set_idx_set
        ...
        if value not in current_available_data_set_idx_set:
            return False

        if value == self.__current_data_set_idx:
            return False

        self.__current_data_set_idx = value
    ...
    async def update_current_symbol_name(
        ...
        # self.__bollinger_upper_band_series = None
        self.__candles_dataframe = None
        self.__current_available_data_set_idx_set = None
        self.__current_data_set_idx = None
        # self.__extreme_lines_array = None
        # self.__extreme_lines_position = None
        # self.__extreme_lines_scale = None
        # self.__line_dataframe_by_level_map.clear()
        ...
        await self.__update_current_available_data_set_idx_set()
        await self.__update_candles_dataframe()
    ...
    async def __update_candles_dataframe(
        ...
        current_data_set_idx = self.__current_data_set_idx

        if current_data_set_idx is None:
            self.__candles_dataframe = None
        ...
        async with self.__candles_dataframe_update_lock:
            ...
            if candles_dataframe is not None:
                min_polars_start_timestamp_ms = candles_dataframe.get_column(
                    'start_timestamp_ms',
                ).max() + 1

                min_start_timestamp_ms = int(
                    min_polars_start_timestamp_ms,
                )
            else:
                min_start_timestamp_ms = 0

            new_candles_dataframe: DataFrame = self.__fetch_candles_dataframe(
                data_set_idx=current_data_set_idx,
                min_start_timestamp_ms=min_start_timestamp_ms,
                symbol_id=current_symbol_id,
            )

            if new_candles_dataframe.height == 0:
                self.__candles_dataframe = None

                return

            high_price_series: Series = new_candles_dataframe.get_column(
                'high_price',
            )

            new_max_trade_price = high_price_series.max()
            ...
            low_price_series: Series = new_candles_dataframe.get_column(
                'low_price',
            )

            new_min_trade_price = low_price_series.min()
        ...
        #
        # logger.info(
        #     f'Trades dataframe was updated by {timer.elapsed:.3f}s'
        # )
        ...
        with Timer() as timer:
            candles_dataframe = polars.read_database_uri(
                ...
                query=(
                    'SELECT'
                    ' *'
                    f' FROM {OKXDataSetRecordData.__tablename__}'
                    ' WHERE'
                    f' symbol_id = {symbol_id.name!r}'
                    f' AND data_set_idx = {data_set_idx!r}'
    ...
    async def __update_current_available_data_set_idx_set(
        self,
    ) -> None:
        current_symbol_name = self.__current_symbol_name
        ...
        current_available_data_set_idx_set: set[int] = set()
        ...
        async with postgres_db_session_maker() as session:
            recursive_cte_full_query = text(
                f"""
WITH RECURSIVE data_set_idx_cte(data_set_idx) AS 
(
  (
    SELECT okx_data_set_record_data.data_set_idx AS data_set_idx 
    FROM okx_data_set_record_data
    WHERE
    symbol_id = {current_symbol_id.name!r}
    ORDER BY
    okx_data_set_record_data.symbol_id ASC
    , okx_data_set_record_data.data_set_idx ASC
    LIMIT 1
  )
  UNION ALL
  SELECT (
    ...
    WHERE
    symbol_id = {current_symbol_id.name!r}
    AND data_set_idx > cte.data_set_idx
    ORDER BY
    symbol_id ASC
    , data_set_idx ASC
    LIMIT 1
  )
  FROM data_set_idx_cte AS cte
  WHERE cte.data_set_idx IS NOT NULL
)
SELECT data_set_idx
FROM data_set_idx_cte
WHERE data_set_idx IS NOT NULL;
                """
            )
            ...
            for row in result:
                data_set_idx: int = row.data_set_idx

                current_available_data_set_idx_set.add(
                    data_set_idx,
                )

        self.__current_available_data_set_idx_set = current_available_data_set_idx_set
...
(
  (
    SELECT okx_data_set_record_data.symbol_id AS symbol_id 
    FROM okx_data_set_record_data ORDER BY okx_data_set_record_data.symbol_id ASC 
    LIMIT 1
  )
  UNION ALL
  SELECT (
...
WHERE symbol_id IS NOT NULL;
    ...
    def __update_velocity_series(
        ...
        self.__velocity_series = candles_dataframe.get_column(
            'total_trades_count',
        )
