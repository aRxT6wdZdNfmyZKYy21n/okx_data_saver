...
from main.offline_inference.artifacts import (
    write_trade_research_inference_meta,
...
from main.offline_inference.paths import (
    ...
    trade_research_inference_npz_path,
...
class TradeResearchDatasetContext:
    symbol_id: str
    df: polars.DataFrame
...
def _run_batch_inference_inference_mode(
    sample_indices: list[int],
    ...
    for chunk_index, chunk_start in enumerate(
        ...
        if (chunk_index + 1) % 10 == 0 or (chunk_index + 1) == total_chunks:
            logger.info(
                'Trade research inference-mode export: %d/%d batches, %d/%d samples',
                chunk_index + 1,
                total_chunks,
                len(inference_by_sample),
                len(sample_indices),
...
def _prepare_trade_research_dataset_context(
    symbol_id: str,
) -> TradeResearchDatasetContext:
    ...
    grid_sample_indices, _sample_selection_note = _sample_indices_for_full_dataset(
    ...
    logger.info(
        'Dataset preparation done: samples=%d start_index=%d level0_rows=%d grid=%d pnl=%d',
        dataset_length,
        start_index,
        level0_height,
        len(grid_sample_indices),
        len(pnl_sample_indices),
    )

    return TradeResearchDatasetContext(
        symbol_id=symbol_id,
        df=df,
        metadata=metadata,
        ...
        level0_to_raw_row_indices=level0_to_raw_row_indices,
        max_sample_index=max_sample_index,
...
def _metadata_fields_from_context(
    context: TradeResearchDatasetContext,
    payload_mode: str,
) -> dict[str, Any]:
    last_bar_row = context.df.row(context.df.height - 1, named=True)
    last_bar_start_trade_id = int(_row_value(last_bar_row, 'start_trade_id'))
    return {
        'run_label': np.array([context.run_label], dtype=object),
        'checkpoint_path': np.array([context.checkpoint_path], dtype=object),
        'policy_path': np.array([context.policy_path], dtype=object),
        'entry_hint_mode': np.array([context.entry_hint_mode], dtype=object),
        'eval_horizon': np.array([context.eval_horizon], dtype=object),
        'dataset_length': np.array([context.dataset_length], dtype=np.int64),
        'start_index': np.array([context.start_index], dtype=np.int64),
        'pnl_stride': np.array([context.pnl_stride], dtype=np.int64),
        'bars_loaded': np.array([int(context.df.height)], dtype=np.int64),
        'level0_rows': np.array([context.level0_height], dtype=np.int64),
        'last_bar_start_trade_id': np.array([last_bar_start_trade_id], dtype=np.int64),
        'research_limit': np.array([context.research_limit], dtype=np.int64),
        'required_rows': np.array([context.required_rows], dtype=np.int64),
        'train_size': np.array([context.train_size], dtype=np.int64),
        'train_size_ratio': np.array([context.train_size_ratio], dtype=np.float64),
        'payload_mode': np.array([payload_mode], dtype=object),
    }
...
def _append_inference_rows_from_batch(
    rows: dict[str, list[Any]],
    sample_indices: list[int],
    inference_by_sample: dict[int, dict[str, object]],
    context: TradeResearchDatasetContext,
    include_train_targets: bool,
    train_sample_index_by_inference_sample: dict[int, int] | None,
) -> None:
    for sample_index in sample_indices:
        ...
        bar_metadata = _bar_metadata_for_sample(
            ...
            level0_to_raw=context.level0_to_raw_row_indices,
            raw_df=context.df,
            level0_height=context.level0_height,
        )

        if include_train_targets:
            if train_sample_index_by_inference_sample is None:
                raise RuntimeError('train_sample_index map required for train export rows')
            train_sample_index = train_sample_index_by_inference_sample[sample_index]
            targets_by_horizon = _target_log2_from_train_sample(
                train_dataset=context.train_dataset,
                train_sample_index=train_sample_index,
                horizon_names=context.horizon_names,
            )
            eval_target_log2 = float(targets_by_horizon[context.eval_horizon])
            stored_train_sample_index = int(train_sample_index)
        else:
            targets_by_horizon = {
                horizon_name: float('nan')
                for horizon_name in context.horizon_names
            }
            eval_target_log2 = float('nan')
            stored_train_sample_index = -1

        rows['sample_index'].append(int(sample_index))
        rows['train_sample_index'].append(stored_train_sample_index)
        rows['eval_target_log2'].append(eval_target_log2)
...
def _empty_export_rows(context: TradeResearchDatasetContext) -> dict[str, list[Any]]:
    rows: dict[str, list[Any]] = {
        'sample_index': [],
        'train_sample_index': [],
        'eval_target_log2': [],
...
def run_trade_research_inference_export(symbol_id: str) -> None:
    context = _prepare_trade_research_dataset_context(symbol_id=symbol_id)
    eval_horizon = context.eval_horizon
    ...
    write_trade_research_inference_meta(
        ...
        payload={
            'status': 'computing',
            'eval_horizon': eval_horizon,
            'pnl_stride': context.pnl_stride,
            'research_limit': context.research_limit,
            'policy_path': context.policy_path,
            'entry_hint_mode': context.entry_hint_mode,
            'payload_mode': 'inference',
        },
    )
    ...
    if _should_rebuild_existing_npz(
        ...
        stack_fingerprint=context.stack_fingerprint,
        start_index=context.start_index,
        payload_mode='inference',
    ):
        existing_npz = None
    ...
    all_sample_indices = context.inference_sample_indices
    samples_to_infer = [
        sample_index
    ...
    logger.info(
        'Trade research inference export: symbol=%s total_samples=%d existing=%d new=%d',
        symbol_id,
    ...
    inference_by_sample = _run_batch_inference_inference_mode(
        sample_indices=samples_to_infer,
        inference_dataset=context.dataset,
        symbol_id=symbol_id,
    )

    rows = _empty_export_rows(context)
    _append_inference_rows_from_batch(
        rows=rows,
        sample_indices=samples_to_infer,
        inference_by_sample=inference_by_sample,
        context=context,
        include_train_targets=False,
        train_sample_index_by_inference_sample=None,
    )

    metadata_fields = _metadata_fields_from_context(
        context=context,
        payload_mode='inference',
    )
    ...
    sample_count = int(merged_payload['sample_index'].shape[0])
    last_sample_index = int(np.max(merged_payload['sample_index']))
    mapped_grid_count = sum(
        1
        for sample_index_value in context.grid_sample_indices
        if sample_index_value in set(merged_payload['sample_index'].astype(np.int64).tolist())
    )
    write_trade_research_inference_meta(
        symbol_id=symbol_id,
        eval_horizon=eval_horizon,
        payload={
            ...
            'pnl_stride': context.pnl_stride,
            'research_limit': context.research_limit,
            'required_rows': context.required_rows,
            'bars_loaded': int(context.df.height),
            'level0_rows': context.level0_height,
            'dataset_length': context.dataset_length,
            'start_index': context.start_index,
            'sample_count': sample_count,
            'last_sample_index': last_sample_index,
            'last_bar_start_trade_id': int(metadata_fields['last_bar_start_trade_id'][0]),
            'run_label': context.run_label,
            'checkpoint_path': context.checkpoint_path,
            'policy_path': context.policy_path,
            'entry_hint_mode': context.entry_hint_mode,
            'payload_mode': 'inference',
            'grid_sample_count': len(context.grid_sample_indices),
            'mapped_grid_count': mapped_grid_count,
            'sample_selection_note': (
                f'inference-mode full export: {mapped_grid_count}/'
                f'{len(context.grid_sample_indices)} grid samples'
            ),
        },
    )
...
def run_trade_research_export_safe(symbol_id: str) -> None:
    ...
    try:
        ...
        run_trade_research_inference_export(symbol_id=symbol_id)
