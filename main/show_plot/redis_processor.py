...
class RedisFinPlotChartProcessor:
    ...
    async def update_trades_dataframe(
        ...
        logger.info(f'Updating trades dataframe for {symbol_id}')
    ...
    async def update_bollinger_series(
        ...
        logger.info(f'Updating bollinger series for {symbol_id}')
        ...
        if bollinger_data is not None:
            ...
            logger.info(f'Updated bollinger series for {symbol_id}')
    ...
    async def update_candle_dataframe_by_interval_name_map(
        ...
        logger.info(f'Updating candle dataframes for {symbol_id}')
    ...
    async def update_rsi_series(
        ...
        logger.info(f'Updating RSI series for {symbol_id}')
        ...
        if rsi_series is not None:
            ...
            logger.info(f'Updated RSI series for {symbol_id}')
    ...
    async def update_trades_smoothed_dataframe_by_level_map(
        ...
        logger.info(f'Updating smoothed dataframes for {symbol_id}')
    ...
    async def update_extreme_lines(
        ...
        logger.info(f'Updating extreme lines for {symbol_id}')
        ...
        if extreme_lines_array is not None:
            ...
            logger.info(f'Updated extreme lines for {symbol_id}')
    ...
    async def update_order_book_volumes(
        ...
        logger.info(f'Updating order book volumes for {symbol_id}')

        (
            asks_array,
            bids_array,
        ) = await self.redis_adapter.load_order_book_volumes_arrays(symbol_id)

        if asks_array is not None and bids_array is not None:
            # Обновляем глобальные переменные
            g_globals.asks_array = asks_array
            g_globals.bids_array = bids_array
            logger.info(f'Updated order book volumes for {symbol_id}')
    ...
    async def update_velocity_series(
        ...
        logger.info(f'Updating velocity series for {symbol_id}')
        ...
        if velocity_series is not None:
            ...
            logger.info(f'Updated velocity series for {symbol_id}')
    ...
    async def load_all_data_for_symbol(
        ...
        logger.info(f'Loading all data for {symbol_id}')

        try:
            # Загружаем все типы данных параллельно
            await polars.asyncio.gather(
                self.update_trades_dataframe(symbol_id, min_trade_id),
                self.update_bollinger_series(symbol_id),
                self.update_candle_dataframe_by_interval_name_map(symbol_id),
                self.update_rsi_series(symbol_id),
                self.update_trades_smoothed_dataframe_by_level_map(symbol_id),
                self.update_extreme_lines(symbol_id),
                self.update_order_book_volumes(symbol_id),
                self.update_velocity_series(symbol_id),
            )

            logger.info(f'Successfully loaded all data for {symbol_id}')
            return True

        except Exception as e:
            logger.error(f'Error loading data for {symbol_id}: {e}')
            return False
