...
def clean_trade_data_batch(args):
    ...
    async def _clean_batch():
            ...
            async with (
                ...
                for trade_id in okx_trade_data_existent_trade_id_array_diff:
                    ...
                    try:
                        ...
                        if cleaned_count % _COMMIT_COUNT == 0:
                            print(
        ...
        print(
            f'[PID {os.getpid()}] Миграция торгов завершена. Мигрировано: {cleaned_count}, пропущено: {skipped_count}, ошибок: {error_count}'
        )
...
def clean_order_book_data_batch(args):
    ...
    async def _clean_batch():
        ...
        for symbol_name in SymbolConstants.IdByName:
            ...
            print(
                f'[PID {os.getpid()}] Начинаем обработку {okx_order_book_data_2_existent_timestamp_ms_array.size} записей торгов...'
            )
            ...
            async with session_factory() as session_read:
                for timestamp_ms in okx_order_book_data_2_existent_timestamp_ms_array:
                    ...
                    if not (-10800000 <= timestamp_ms <= 1767214800000):
                        print(
                            f'Found incorrect timestamp (ms): {timestamp_ms!r}'
                    ...
                    if cleaned_count % _COMMIT_COUNT == 0:
                        print(
                            f'[PID {os.getpid()}] Проверено {cleaned_count} записей order book...'
                        )
        ...
        print(
            f'[PID {os.getpid()}] Миграция order book завершена. Мигрировано: {cleaned_count}, пропущено: {skipped_count}, ошибок: {error_count}'
        )
        return cleaned_count
...
def clean_candle_data_15m_batch(args):
        ...
        async with (
            ...
            async for candle in result.scalars():
                ...
                if cleaned_count % _COMMIT_COUNT == 0:
                    ...
                    print(
                        f'[PID {os.getpid()}] Зафиксировано {cleaned_count} записей свечей 15m...'
                    )
...
def clean_candle_data_1h_batch(args):
        ...
        async with (
            ...
            async for candle in result.scalars():
                ...
                if cleaned_count % _COMMIT_COUNT == 0:
                    await session_write.commit()
                    print(
                        f'[PID {os.getpid()}] Зафиксировано {cleaned_count} записей свечей 1H...'
                    )
...
class DatabaseMigrator:
    ...
    async def clean_trade_data(self):
    ...
    async def clean_order_book_data(self):
        ...
        try:
            ...
            # Запускаем многопроцессную миграцию
            with ProcessPoolExecutor(max_workers=_MAX_WORKERS) as executor:
                results = list(executor.map(clean_order_book_data_batch, args_list))
    ...
    async def clean_candle_data_15m(self):
        ...
        # Запускаем многопроцессную миграцию
        with ProcessPoolExecutor(max_workers=_MAX_WORKERS) as executor:
            results = list(executor.map(clean_candle_data_15m_batch, args_list))

        total_cleaned = sum(results)
        logger.info(
            f'Миграция свечей 15m завершена. Всего мигрировано: {total_cleaned} записей'
        )

    async def clean_candle_data_1h(self):
        ...
        # Запускаем многопроцессную миграцию
        with ProcessPoolExecutor(max_workers=_MAX_WORKERS) as executor:
            results = list(executor.map(clean_candle_data_1h_batch, args_list))

        total_cleaned = sum(results)
        logger.info(
            f'Миграция свечей 1H завершена. Всего мигрировано: {total_cleaned} записей'
        )
    ...
    async def run_migration(self):
        ...
        try:
            ...
            await asyncio.gather(
                ...
                self.clean_order_book_data(),
                # self.clean_candle_data_15m(),
