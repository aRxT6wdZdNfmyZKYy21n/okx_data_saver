...
try:
    ...
    from main.process_data.cpp_data_processor_wrapper import (
        g_cpp_data_processor_wrapper,
    )
...
class PerformanceBenchmark:
    ...
    async def benchmark_cpp_processor(self, trades_df: pl.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            return {'success': False, 'error': str(e), 'processor_type': 'cpp'}

    async def benchmark_hybrid_processor(
        ...
        try:
            ...
            return {'success': False, 'error': str(e), 'processor_type': 'hybrid'}

    async def benchmark_python_simulation(
        self, trades_df: pl.DataFrame
    ) -> dict[str, Any]:
        """
    ...
    async def run_single_benchmark(
        self, num_trades: int, num_iterations: int = 3
    ) -> dict[str, Any]:
        """
        ...
        """
        logger.info(
            f'Running benchmark for {num_trades} trades, {num_iterations} iterations'
        )
    ...
    def print_results(self, results: dict[str, Any]) -> None:
        ...
        if 'summary' in results:
            ...
            if 'cpp_vs_python_speedup_mean' in summary:
                print(
                    f'C++ vs Python speedup: {summary["cpp_vs_python_speedup_mean"]:.2f}x'
                )

            if 'hybrid_vs_python_speedup_mean' in summary:
                print(
                    f'Hybrid vs Python speedup: {summary["hybrid_vs_python_speedup_mean"]:.2f}x'
                )

            if 'cpp_vs_hybrid_speedup_mean' in summary:
                print(
                    f'C++ vs Hybrid speedup: {summary["cpp_vs_hybrid_speedup_mean"]:.2f}x'
                )
